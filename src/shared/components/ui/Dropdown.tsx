     ...
     <motion.div className={styles.dropdown} onClick={handleDropdownClick}>
        <div style={{ display: 'flex', alignItems: 'center', userSelect: 'none' }}>
           ...
           <svg color="var(--color-border)"
                style={{ marginLeft: 10, width: 17, fill: 'var(--color-border)', rotate: `${clickCount * 180}deg` }}
                xmlns="http://www.w3.org/2000/svg" shapeRendering="geometricPrecision" textRendering="geometricPrecision"
                imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 512 298.04">
        ...
        <motion.div onClick={e => e.stopPropagation()}
                    style={{
                       marginTop: isOpened ? '15px' : '5px', cursor: 'default', overflow: 'hidden',
                       transition: '0.2s all', maxHeight: `${isOpened ? '220px' : 0}`, textAlign: 'justify',
                    }}>
           {content}
        </motion.div>
     </motion.div>
