import React, { AnimationEventHandler, CSSProperties, FC } from 'react';
...
const Icon: FC<Props> = ({ icon, size, color, style, className, onAnimationEnd }) => {
...
const ICONS = (size: number, color: string, style: CSSProperties, className: string, onAnimationEnd: AnimationEventHandler<SVGSVGElement> | undefined) => {
   return {
      pause: {
         icon: (
           <svg style={{ fill: color, width: size * 1.2, height: size, ...style }} className={className} fill={color} version="1.1" id="Layer_1"
                x="0px" y="0px"
      ...
      chevronDown: {
         icon: (
           <svg onAnimationEnd={onAnimationEnd} className={className} style={{ fill: color, ...style }} xmlns="http://www.w3.org/2000/svg"
                width={size} height={size} shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality"
                fillRule="evenodd" clipRule="evenodd" viewBox="0 0 512 298.04">
      ...
      sixDots: {
         icon: (
           <svg xmlns="http://www.w3.org/2000/svg" style={{ width: size, aspectRatio: '1 / 1', ...style }} fill={color} viewBox="0 0 15 15">
